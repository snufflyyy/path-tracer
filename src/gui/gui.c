...
void gui_update(GUI* gui, Camera* camera, World* world) {
  ...
  igBegin("Camera", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize);
    ...
    if (igSmallButton("Reset")) { reset_camera_framebuffer = true; }
  igEnd();

  igBegin("World", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize);
...
void gui_render(GUI* gui, Color* framebuffer, u32 sample_count, u32 width, u32 height) {
  ...
  for (usize i = 0; i < framebuffer_length; i++) {
    gui->framebufferRGB[i] = color_convert_to_rgb(color_linear_to_gamma(color_scale(framebuffer[i], 1.0f / sample_count)));
  }
